...
enemies = []
car1_vel = 5  # Base value: 5
car2_vel = 6  # Base value: 6
car3_vel = 5  # Base value: 5
car4_vel = 4  # Base value: 4
car5_vel = 5  # Base value: 5
...
def main():
        ...
        for x in range(len(enemies)):
            ...
            if frogger.y - player_velocity in range(enemies[x].y - y_check, enemies[x].y + y_check):
                if frogger.x in range(future_check - x_check, future_check + x_check):
                    hit = True
                    if frogger.y == enemies[x].y:
                        side_hit = True
                    if frogger.x < enemies[x].x:
                        pos_right = True
        return hit, pos_right, side_hit
    ...
    def action():
        ...
        for n in range(0, 16):
            ...
            actions[4 * n] = ai.reward(frogger.x, frogger.y, width, height)[0] + ai.victory_bias
            actions[(4 * n) + 1] = ai.reward(frogger.x, frogger.y, width, height)[1]
            actions[(4 * n) + 2] = ai.reward(frogger.x, frogger.y, width, height)[2]
            actions[(4 * n) + 3] = ai.reward(frogger.x, frogger.y, width, height)[0]
    ...
    while running:
        ...
        if AI_mode:
            ...
            for i in range(len(node_arr)):
                if best_val == node_arr[4 * i]:
                    frogger.y -= player_velocity
                    redraw()
                elif best_val == node_arr[(4 * i) + 3]:
                    frogger.y += player_velocity
                    redraw()
                elif best_val == node_arr[(4 * i) + 2]:
                    frogger.x += player_velocity
                    redraw()
                elif best_val == node_arr[(4 * i) + 1]:
                    frogger.x -= player_velocity
                    redraw()
